...
public class ATC extends Thread {
    ...
    public static volatile int planeLeft = 0;
    ...
    public void acquireGateAndRunway(Plane plane) {
        try {
            ...
            gate.acquire();
            System.out.println(ANSI_YELLOW_BACKGROUND + "ATC: " + plane.getPlaneName() + " has acquire a gate for landing." + ANSI_RESET);
            System.out.println(ANSI_YELLOW_BACKGROUND + "ATC: " + plane.getPlaneName() + " Attempting to acquire the runway for landing." + ANSI_RESET);
            runway.acquire();
            System.out.println(ANSI_YELLOW_BACKGROUND + "ATC: " + plane.getPlaneName() + " has acquire a runway for landing." + ANSI_RESET);
    ...
    public void acquireRunway(Plane plane) {
        try {
            ...
            runway.acquire();
            System.out.println(ANSI_YELLOW_BACKGROUND + "ATC: " + plane.getPlaneName() + " has acquire a runway for departure." + ANSI_RESET);
        } catch (InterruptedException ex) {
            Logger.getLogger(ATC.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
